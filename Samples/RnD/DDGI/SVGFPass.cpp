      ...
      mEnableSpatialVarianceEstimation(true)
{
    ...
    mPrevLinearZTexture = Texture::create2D(width, height, ResourceFormat::RGBA32Float, 1, 1, nullptr, ResourceBindFlags::ShaderResource | ResourceBindFlags::RenderTarget);
    ...
    Texture::SharedPtr normalDepth)
{
    ...
    for (uint32_t i = 0; i < mAtrousIterations; ++i)
    {
        ...
        if (i == std::min(mFeedbackTap, mAtrousIterations-1))
        {
            renderContext->blit(output->getColorTexture(0)->getSRV(), mLastFilteredFbo->getColorTexture(0)->getRTV(), uvec4(-1), uvec4(-1), Sampler::Filter::Point);
        }
    ...
    renderContext->blit(mGBufferInput.linearZ->getSRV(), mPrevLinearZTexture->getRTV(), uvec4(-1), uvec4(-1), Sampler::Filter::Point);
...
{
    GPU_EVENT(renderContext, "TemporalReprojection");
...
{
    GPU_EVENT(renderContext, "SpatialVarianceEstimation");
...
{
    GPU_EVENT(renderContext, "AtrousFilter");
...
void SVGFPass::RenderGui(Gui* gui, const char* group)
{
    if (gui->beginGroup(group))
    {
